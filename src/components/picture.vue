    ...
    <div class="ve-dialog" v-show="showPopup" @click.self="hideDialog">
        <div :class="$style.wrap">
            <div class="ve-dialog-header">
                {{ lang.title
                }}<a href="javascript:;" class="ve-close" @click="hideDialog"
                    >&times;</a
                >
            </div>
            ...
            <div class="ve-dialog-footer">
                <div class="ve-btn-box">
                    <button class="ve-btn" @click="hideDialog">
                        {{ lang.cancel }}
                    </button>
                    <button class="ve-btn" @click="certainHandler">
                        {{ lang.ok }}
                    </button>
                </div>
            </div>
...
export default {
    data() {
        return {
            ...
            customUpload: getConfig("customUpload"),
            customUploadHandler: getConfig("customUploadHandler"),
        };
    },
    computed: {
        showPopup: function () {
        ...
        showPopup: function (val) {
            if (val) {
                if (this.customUpload == true) {
                    this.customUploadHandler.show((href) => {
    ...
    methods: {
        ...
        changeHandler() {
            ...
            if (navigator.userAgent.indexOf("MSIE") >= 1) {
                ...
                if (
                    obj.files.length !== 0 &&
                    obj.files.item(0).type.indexOf("image") !== -1
                ) {
            ...
            if (this.url) {
                if (this.$parent.upload) {
                    ...
                    xhr.onload = function () {
                        this.$store.dispatch("execCommand", {
                            name: "insertHTML",
                            value: `<img src="${xhr.responseText}">`,
                        });
                        this.hideDialog();
                    }.bind(this);
